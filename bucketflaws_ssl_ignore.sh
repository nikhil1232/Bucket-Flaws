...
echo -e "\n\e[30;48;5;82mTesting for Directory Listing . . .${RESET}\n"
a=$(aws s3 ls s3://$bucket --recursive --no-verify-ssl 2>&1 | sed '/InsecureRequestWarning/d')
if [[ $a == *"An error"* ]]; then
  ...
  then
    ...
    down=$(aws s3 sync s3://$bucket $bucket --no-verify-ssl --quiet 2>&1 | sed '/InsecureRequestWarning/d')
    down2=$(ls $PWD/$bucket/)
...
x=$(aws s3api get-bucket-acl --bucket $bucket --no-verify-ssl 2>&1 | sed '/InsecureRequestWarning/d')
if [[ $x == *"An error"* ]]; then
  echo -e "${RED}Failed!!!${RESET}\n"
else
...
echo -e "\n\e[30;48;5;82mTesting for file upload . . .${RESET}\n"
y=$(aws s3 cp bucket.png s3://$bucket --no-verify-ssl 2>&1 | sed '/InsecureRequestWarning/d')
...
then
   ...
   ac=$(aws s3api put-bucket-acl --bucket $bucket --grant-full-control uri=http://acs.amazonaws.com/groups/global/AllUsers --no-verify-ssl 2>&1 | sed '/InsecureRequestWarning/d')
   if [[ $ac == *"An error"* ]]; then
     echo -e "${RED}Failed!!!${RESET}\n"
   else
...
if [[ $obj == "yes" ]] || [[ $obj == "y" ]]; then
  ...
  read obj2
  x1=$(aws s3api get-object-acl --bucket $bucket --key $obj2 --no-verify-ssl 2>&1 | sed '/InsecureRequestWarning/d')
  x2=$(aws s3api head-object --bucket $bucket --key $obj2 --no-verify-ssl 2>&1 | sed '/InsecureRequestWarning/d')
  ...
  if [[ $obj3 == "yes" ]] || [[ $obj3 == "y" ]]; then
    dump=$(aws s3 cp s3://$bucket/$obj2 $bucket/$obj2 --no-verify-ssl 2>&1 | sed '/InsecureRequestWarning/d')
  ...
  if [[ $obj4 == "yes" ]] || [[ $obj4 == "y" ]]; then
    ac2=$(aws s3api put-object-acl --bucket $bucket --key $obj2 --grant-full-control uri=http://acs.amazonaws.com/groups/global/AllUsers --no-verify-ssl 2>&1 | sed '/InsecureRequestWarning/d')
